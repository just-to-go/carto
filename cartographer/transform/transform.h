...
/*
  旋转矩阵到旋转向量的变换,刚好是旋转向量到旋转矩阵变换的逆过程
*/ 
template <typename T>
Eigen::Matrix<T, 3, 1> RotationQuaternionToAngleAxisVector(
  ...
  /*
    normalized_quaternion.w() = cos(th/2) < 0 ==> th > 180 or th <-180 需要反向旋转: th -= 360 or th += 360
    因此,四元数,都取负值
  */ 
  ...
  /*
    q = w + v = cos(th/2) + sin(th/2)*u
      normalized_quaternion.vec().norm() = |sin(th/2)|
      normalized_quaternion.w() = cos(th/2)
      atan2(normalized_quaternion.vec().norm(), normalized_quaternion.w()) = atan(|sin(th/2)|,cos(th/2))
  */ 
  const T angle =
...
Eigen::Quaternion<T> AngleAxisVectorToRotationQuaternion(
  ...
  /*
    angle_axis.squaredNorm() 表示L2范数,等于 angle_axis * angle_axis
    norm L2范数的平方根
    q = cos(th/2) + sin(th/2)* u
    cos(th/2) = cos(norm / 2.) = w
    sin(th/2) = sin(norm / 2.)
    u = angle_axis / norm
  */
  ...
  /*
    计算四元数虚部
      quaternion_xyz = = scale * angle_axis = sin(norm / 2.) * angle_axis / norm
  */ 
