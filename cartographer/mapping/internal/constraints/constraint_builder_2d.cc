...
if(flag == false)
{
  ...
  LOG(WARNING)
    ...
    <<"\n options_.sampling_ratio() = "<<options_.sampling_ratio();
}

  // 添加约束时，距离限制，距离太远，不添加约束
...
/*
  一个节点约束添加完成之后,执行该 任务 .对完成添加约束的节点,进行计数
*/ 
void ConstraintBuilder2D::NotifyEndOfNode() {
  ...
  // 添加完计数任务后,重新初始化 finish_node_task_
  finish_node_task_ = absl::make_unique<common::Task>();

  /*
    为 when_done_task_最优化任务,添加依赖任务
  */ 
...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  /*
    initial_relative_pose = Tsub_node
    ComputeSubmapPose(*submap) = Tmap_sub
    initial_pose = Tmap_sub * Tsub_node = Tmap_node
  */ 
  ...
  ceres_scan_matcher_.Match(pose_estimate.translation(), pose_estimate,
                            ...
                            *submap_scan_matcher.grid, &pose_estimate,
                            &unused_summary);
  /*
    ComputeSubmapPose(*submap).inverse() = Tsub_map
    pose_estimate = Tmap_node 
    constraint_transform = Tsub_map * Tmap_node = Tsub_node 
  */ 
