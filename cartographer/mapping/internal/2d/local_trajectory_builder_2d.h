...
class LocalTrajectoryBuilder2D {
 public:
  ...
  struct MatchingResult {                           //匹配结果结构体
    common::Time time;                              //时间
    transform::Rigid3d local_pose;                  //
    sensor::RangeData range_data_in_local;          //传感器数据
 ...
 private:
  ...
  const proto::LocalTrajectoryBuilderOptions2D options_;  //参数配置
  ActiveSubmaps2D active_submaps_;     //同时维护 2个 submap

  MotionFilter motion_filter_;                                  //运动过滤器
  scan_matching::RealTimeCorrelativeScanMatcher2D
      real_time_correlative_scan_matcher_;                      //实时扫描匹配
  scan_matching::CeresScanMatcher2D ceres_scan_matcher_;        //ceres方法匹配

  std::unique_ptr<PoseExtrapolator> extrapolator_;              //轨迹推测器，融合IMU 和 Odometry

  int num_accumulated_ = 0;                                     // 累计数据量
  sensor::RangeData accumulated_range_data_;                    //该轨迹的累计数据
  ...
  // 采集雷达的点云数据 
  RangeDataCollator range_data_collator_;                       //搜集传感器数据
};
