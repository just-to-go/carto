...
PoseGraph2D::PoseGraph2D(
  ...
  LOG(WARNING)
  <<"\n PoseGraph2D init:"
  <<"\n options.has_overlapping_submaps_trimmer_2d() = "<<options.has_overlapping_submaps_trimmer_2d();
...
/**
 * optimization_problem_->AddSubmap() 添加子地图的全局位姿
 * **/ 
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  if (insertion_submaps.size() == 1) {
    ...
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
      ...
      // optimization_problem_->AddSubmap() 添加 子地图原点的全局坐标
...
NodeId PoseGraph2D::AppendNode(
  ...
  /**
   * data_.trajectory_nodes 节点数据,添加节点数据
   *  constant_data 节点数据,节点局部坐标,节点点云数据
   *  optimized_pose 节点全局坐标
   * 
   * 返回:IdType{trajectory_id, index}  trajectory_id轨迹号, index trajectory_nodes节点编号
   * ***/ 
  const NodeId node_id = data_.trajectory_nodes.Append(
      trajectory_id, TrajectoryNode{constant_data, optimized_pose});

  /**
   * data_.num_trajectory_nodes 节点数量, +1
   * ***/ 
  ++data_.num_trajectory_nodes;
  // Test if the 'insertion_submap.back()' is one we never saw before.
  /**
   * data_.submap_data 没有数据 或者 该轨迹最后一个submap 不是 insertion_submaps.back() 时,插入该子图
   * **/ 
  if (data_.submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0 ||
    ...
    /**
     * data_.submap_data 子地图数据
     *    data_.submap_data.Append(trajectory_id, InternalSubmapData()); 添加一个空子图
     *    data_.submap_data.at(submap_id).submap = insertion_submaps.back();  再给空子图进行赋值
     * **/ 
...
NodeId PoseGraph2D::AddNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data,// 一帧点云数据
    const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) {
  //生成一个Pose(相对于世界坐标系的绝对坐标)，轨迹相对于世界坐标系的Pose × 该节点相对于该轨迹的坐标
  /**
   * GetLocalToGlobalTransform() => 获取局部到全局的坐标变换
   * optimized_pose = GetLocalToGlobalTransform() * constant_data->local_pose => 获取节点全局坐标
   * constant_data->local_pose = pose_estimate 激光匹配计算出的最优位姿 Tmap_base
   * **/ 
  const transform::Rigid3d optimized_pose(
  ...
  // LOG(ERROR)
  //   <<"\n 获取节点的世界坐标："
  //   <<"\n constant_data->local_pose.x = "<<constant_data->local_pose.translation().x()
  //   <<"\n constant_data->local_pose.y = "<<constant_data->local_pose.translation().y()
  //   <<"\n optimized_pose.x = "<<optimized_pose.translation().x()
  //   <<"\n optimized_pose.y = "<<optimized_pose.translation().y();
    optimized_pose.DebugString();

  // 添加节点 NodeId 和 子地图 。将该节点 加入到 PoseGraph2D 维护的 trajectory_nodes_ 容器中。
  // 给 data_ 赋值
  const NodeId node_id = AppendNode(constant_data, trajectory_id,
                                    insertion_submaps, optimized_pose);
  // We have to check this here, because it might have changed by the time we
  // execute the lambda.
  //检测正在维护的submap中的第一个submap 是否 finished 
  ...
  if (!global_localization_samplers_[trajectory_id]) {
    global_localization_samplers_[trajectory_id] =
      ...
      LOG(WARNING)
        ...
        <<"\n global_localization_samplers_ init:"
  ...
  AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
    ...
    if (CanAddWorkItemModifying(trajectory_id)) {
      for (const auto& observation : landmark_data.landmark_observations) {
        // 将观测到的标签数据,添加到 data_ 中.其中一个 id ,可以对应多个观测数据
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  const Submap2D* submap;
  {
    ...
    //比较该节点 time 和与该子地图匹配的上一个节点时间，取其中最大的时间
...
if(flag == false)
{
  ...
  LOG(WARNING)
    ...
    // 同一条轨迹 或者　节点时间　距离上一次关联时间　小于一定时间差时，进行局部约束
  ...
  // 局部约束，在初始点附近一定范围内，进行约束
  if (maybe_add_local_constraint) {
    //计算出 optionmizition_problem 中节点坐标世界坐标和地图世界坐标之间的关系（点在对应子地图中的坐标）
    /*
      子地图在优化后地图中的坐标
        global_pose.inverse() = Tmap_sub
      节点在优化后地图中的坐标
        global_pose_2d = Tmap_node
      节点相对子地图坐标
        initial_relative_pose = Tsub_map * Tmap_node = Tsub_node
    */ 
    ...
    /*
      使用分支定界加速，来计算约束
        initial_relative_pose为预测值，通过匹配扫描，来获取node与submap之间的最优相对关系
    */
    constraint_builder_.MaybeAddConstraint(
        submap_id, submap, node_id, constant_data, initial_relative_pose);
  } 
  else if (maybe_add_global_constraint) //全局约束，没有初始点，在全局进行搜索，计算约束
  {
    // 使用分支定界加速，来计算约束
...
// 为节点 添加约束
/**
 * ComputeConstraintsForNode,为节点添加约束:
 *    optimization_problem_,添加submap 和 node
 *    节点约束:
 *        node与活跃submap之间约束
 *        node与之前完成的 submap 之间约束
 *        新完成的 submap 与所有之前的 node 之间的约束
 * **/ 
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  std::set<NodeId> newly_finished_submap_node_ids;
  {
    ...
    //根据节点数据的时间获取最新的submap的id,将 submap 添加到 optimization_problem_ 中。
    // 返回活跃子地图
    /**
     * optimization_problem_.submap_data_ 赋值
     * 返回活跃状态的两张子地图
     * **/ 
    ...
    /*
      计算节点前端建图时的坐标.
        constant_data->local_pose = Tmap_base
        constant_data->gravity_alignment.inverse() = Rmap_base^-1 = Rbase_baseg
        local_pose_2d = Tmap_base * Rbase_baseg = Tmap_baseg 将坐标旋转变换到,与map坐标系对齐
    */
    ...
    /*
      优化后子地图的坐标:
        optimization_problem_->submap_data().at(matching_id).global_pose = Tmap'_sub
      优化前子地图的坐标:
        constraints::ComputeSubmapPose(*insertion_submaps.front()) = Tmap_sub

      global_pose_2d = Tmap'_sub * Tsub_map * Tmap_baseg = Tmap'_baseg
    */
    ...
    //把该节点信息加入到  optimization_problem_ 中
    /**
     * optimization_problem_.node_data 添加轨迹节点
     * local_pose_2d 重力对齐后的坐标
     * **/ 
    ...
    //遍历当前活跃的 submap.然后 计算当前节点与 submap的相对位置即约束,压入到 data_.constraints
    for (size_t i = 0; i < insertion_submaps.size(); ++i) {
      ...
      /*
        ...
        重力对齐后的节点坐标.在没有误差的情况下,重力对齐后的节点姿态方向跟地图原点的方向是一致的,但是由于误差,其角度还是有一定的差距
          local_pose_2d
      */ 
      ...
      LOG(ERROR)
          <<"\n constant_data->local_pose = "<<constant_data->local_pose.cast<float>()
          <<"\n local_pose_2d = "<<local_pose_2d.cast<float>()
          <<"\n constraint_transform = "<<constraint_transform.cast<float>();

      //把约束 压入约束集合中．　Constraint::INTRA_SUBMAP　已经插入的节点约束
    ...
    // them out here.
    //遍历所有历史中的submap，找出完成子地图构建，并且与该节点，没有产生约束的子地图，压入　finished_submap_ids　中．
    ...
    //若，true 维护的两个 submap 中的第一个子地图建图完成.取出来,并设置其状态为 SubmapState::kFinished 
  ...
  //计算该节点与所有未产生约束submap之间的约束。通过扫面匹配来获取约束
  for (const auto& submap_id : finished_submap_ids) {
    ComputeConstraint(node_id, submap_id);
  }

  //取出刚建图完成的子地图. 与 所有之前没有建立约束的node,进行匹配计算之间约束
  if (newly_finished_submap) {
    const SubmapId newly_finished_submap_id = submap_ids.front();
    ...
    for (const auto& node_id_data : optimization_problem_->node_data()) {
      ...
      if (newly_finished_submap_node_ids.count(node_id) == 0) {
        ComputeConstraint(node_id, newly_finished_submap_id);//计算约束
      }
    }
  }
  /*
      节点约束计算完成之后,设置完成节点数量约束值+1 的 任务
  */
  constraint_builder_.NotifyEndOfNode();
  absl::MutexLock locker(&mutex_);
  // 记录添加的节点数量，每次全局优化之后，会清零
  ++num_nodes_since_last_loop_closure_;
  LOG(ERROR)<<"\n run ComputeConstraintsForNode:"
  ...
  if (options_.optimize_every_n_nodes() > 0 &&
    ...
    LOG(WARNING)
      ...
      <<"\n num_nodes_since_last_loop_closure_ = "<<num_nodes_since_last_loop_closure_
      <<"\n options_.optimize_every_n_nodes() = "<<options_.optimize_every_n_nodes();
    return WorkItem::Result::kRunOptimization;
  }
  return WorkItem::Result::kDoNotRunOptimization;
}

// 获取该节点 和 与该submap 产生约束的最近的 node 之间的最大时间
...
void PoseGraph2D::HandleWorkQueue(
  ...
  {
    absl::MutexLock locker(&mutex_);
    //在队列后面 插入约束, data_ 中记录所有的约束
    //全局优化,是针对所有的约束 进行的优化
    data_.constraints.insert(data_.constraints.end(), result.begin(),
                             result.end());
  }
  //执行全局优化,并用优化结果来更新 data_ 数据
  RunOptimization();
...

  {
    ...
    count++;
    LOG(ERROR)
    <<"\n run HandleWorkQueue time : "<<count
    <<"\n num_nodes_since_last_loop_closure_ = "<<num_nodes_since_last_loop_closure_;
...
void PoseGraph2D::RunOptimization() {
  ...
  // 所有的约束，所有轨迹状态，所有的 landmark,一起进行全局约束
  optimization_problem_->Solve(data_.constraints, GetTrajectoryStates(),
                               data_.landmark_nodes);
  absl::MutexLock locker(&mutex_);

  const auto& submap_data = optimization_problem_->submap_data();//获取优化后的 submap_data
  const auto& node_data = optimization_problem_->node_data();//获取优化后的 node_data

  //遍历所有的轨迹
  for (const int trajectory_id : node_data.trajectory_ids()) 
  {
    //遍历轨迹中的所有节点,还原节点优化后的全局坐标
    for (const auto& node : node_data.trajectory(trajectory_id)) 
    {
      auto& mutable_trajectory_node = data_.trajectory_nodes.at(node.id);
      /*
        优化前的trajectory_node 是姿态对齐后的节点坐标.
        node.data.global_pose_2d = Tmap_node' 优化后的也是姿态对齐后的坐标
        mutable_trajectory_node.constant_data->gravity_alignment = Rmap_node
      */ 
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
    ...
    const int trajectory_id) const {
      /*
        begin_it 第一个submap_pose
        end_it 最后一个submap_pose
      */ 
  ...
  /**
   * 
   * 子地图原点在优化后的地图坐标系(map')下坐标:
   *    data_.submap_data.at(last_optimized_submap_id).submap->local_pose() = Tmap'_sub1
   * 子地图在原地图坐标(map)下坐标
   *    global_submap_poses.at(last_optimized_submap_id).global_pose) = Tmap_sub1
   * 返回优化前到优化后的坐标变换:
   *    Tmap'_sub1 * Tmap_sub1^-1 = Tmap'_sub1 * Tsub1_map = Tmap'_map 
   * **/ 
