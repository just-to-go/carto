...
bool MotionFilter::IsSimilar(const common::Time time,
  ...
  LOG_IF_EVERY_N(INFO, num_total_ >= 500, 500)
    ...
    {
        LOG(WARNING)
            <<"\n MotionFilter::IsSimilar : "
            <<"\n options_.max_time_seconds() = "<<options_.max_time_seconds()
            <<"\n options_.max_distance_meters() = "<<options_.max_distance_meters()
            <<"\n options_.max_angle_radians() = "<<options_.max_angle_radians();
        flag = true;
    }
    // 接收到雷达帧总数
  ++num_total_;
