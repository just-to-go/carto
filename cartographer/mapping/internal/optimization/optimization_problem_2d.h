...
struct NodeSpec2D {
  common::Time time;                    //该节点时间戳
  transform::Rigid2d local_pose_2d;     //该节点前端建图中的地图坐标
  transform::Rigid2d global_pose_2d;    //该节点基于全局优化后的地图坐标
  Eigen::Quaterniond gravity_alignment; //该节点的方向四元数
};
...
class OptimizationProblem2D
 ...
 private:
  ...
  // imu数据,3D建图时使用;2D建图,不添加IMU数据
  sensor::MapByTime<sensor::ImuData> empty_imu_data_;

  // 里程计数据,处理 odom 数据时,添加
