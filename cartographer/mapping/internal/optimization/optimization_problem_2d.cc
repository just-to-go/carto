  ...
  for (const auto& landmark_node : landmark_nodes) {
    for (const auto& observation : landmark_node.second.landmark_observations) {
      ...
      if (!C_landmarks->count(landmark_id)) {
        ...
        // 不同的 landmark_id 以第一帧数据作为初始值,进行初始化.
        C_landmarks->emplace(
            landmark_id,
...
/**
 * observation 提供时刻 和 landmark_to_tracking_transform_ 坐标变换
 * 根据 时刻,通过均值插值方式,知道该时刻的map坐标,然后 通过 landmark_to_tracking_transform_ 转换到 landmark坐标,作为参考值
 * 然后通过 然后计算 C_landmarks->at(landmark_id).rotation() 和 C_landmarks->at(landmark_id).translation() 与参数值的残差.
 * **/ 
...
void OptimizationProblem2D::Solve(
    const std::vector<Constraint>& constraints,//所有约束
    const std::map<int, PoseGraphInterface::TrajectoryState>&
        trajectories_state,//所有轨迹状态
    const std::map<std::string, LandmarkNode>& landmark_nodes) //所有路标节点
    {
  if (node_data_.empty()) {
    // Nothing to optimize.
    return;
  ...
  /*
    优化变量:
      ...
      C_landmarks     标记位置
  */ 
  MapById<SubmapId, std::array<double, 3>> C_submaps;//记录所有子地图的全局位姿
  MapById<NodeId, std::array<double, 3>> C_nodes;
  std::map<std::string, CeresPose> C_landmarks;
  bool first_submap = true;
  //遍历所有的submap, 设置其全局坐标为 优化参数.
  for (const auto& submap_id_data : submap_data_) {
    ...
    */ 
    ...
    if (first_submap || frozen) {
      ...
      // trajectory.
      // 若冻结或者第一帧，则将其设为常量，在优化过程中，不在改变其值
  ...
  for (const auto& node_id_data : node_data_) {
    ...
    if (frozen) {
      //若冻结，则设置成 常量.对在冻结的轨迹上的节点,添加约束
      problem.SetParameterBlockConstant(C_nodes.at(node_id_data.id).data());
    }
  }
  // Add cost functions for intra- and inter-submap constraints.
  //遍历所有约束,添加对应node节点和子地图submap之间的约束
  for (const Constraint& constraint : constraints) {
    /*
      误差计算，用node的全局坐标和submap的全局坐标，计算出相对坐标,然后与 constraint.pose 作差来计算误差项。在全局优化过程中，不断调整使得，整体误差最小
      constraint.pose = Tsub_node 节点相对子地图的坐标
    */ 
    problem.AddResidualBlock(
  ...
  /**
   * 添加 landmark 约束,添加思路:
   *  根据时间戳,找到landmark前后两帧的node,然后通过时间戳进行均线插值,找到landmark 理论位置.
   *  计算landmark测量位置 与 理论位置之间的误差,作为残差项,进行优化
   * **/ 
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      /**
       * 添加两个节点之间约束:
       *    以两节点时间,计算其对应的odom坐标,以其相对odom坐标作为参考,以两节点 node 全局坐标作为变量,来添加约束.
       * ***/ 
      ...
      /**
       * 添加两节点之间约束:
       *    以 局部建图得到的两节点之间相对 pose 作为参考,优化node点,优化过程中,node变化,保持其相对坐标尽可能保持这个相对关系
       * 
       *    first_node_data.local_pose_2d.inverse() = Tnode1_map
       *    second_node_data.local_pose_2d = Tmap_node2
       *    relative_local_slam_pose = Tnode1_map * Tmap_node2 = Tnode1_node2
       * **/ 
  ...
  */ 
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (; node_it != trajectory_end; ++node_it) {
      ...
      /*
        获取 node_data.time 时刻的 fixed 位姿 Tfixed_node
      */ 
      ...
      if (!fixed_frame_pose_initialized) {
        ...
        if (trajectory_data.fixed_frame_origin_in_map.has_value()) {
          ...
          /*
            node_data.global_pose_2d = Tmap_node
            constraint_pose.zbar_ij = Tfixed_node
            fixed_frame_pose_in_map = Tmap_node * Tnode_fixed = Tmap_fixed
          */ 
      ...
      /**
       * constraint_pose 为 node_id 时刻的 fixed坐标
       * C_fixed_frames.at(trajectory_id).data() 为 fixed原点 在 map坐标系下的坐标 Tmap_fixed
       * 以 constraint_pose 为参考.将 C_nodes.at(node_id).data() 转换到 fixed 坐标系下,与 参考做残差.
       * **/ 
      problem.AddResidualBlock(
  ...
  if (odometry_data_.HasTrajectory(trajectory_id)) {
    ...
    */ 
    ...
    if (first_node_odometry != nullptr && second_node_odometry != nullptr) {
      /*
          first_node_data.gravity_alignment = Rnode1
          first_node_odometry->inverse() = Tnode1_map
          second_node_odometry = Tmap_node2
          econd_node_data.gravity_alignment.inverse() = Rnode2^-1
          relative_odometry 为前后两坐标,姿态重力对齐后的相对坐标
      */ 
      ...
      LOG(WARNING)<<"\n relative_odometry = "<<relative_odometry.cast<float>();
