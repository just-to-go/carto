...
inline uint16 BoundedFloatToValue(const float float_value,
  ...
  /*
     value = (float_value - lower_bound) * 32766 / (upper_bound - lower_bound) + 1
  */ 
  const int value =
      common::RoundToInt(
...
constexpr float kMinProbability = 0.1f;// 最小概率 0.1
constexpr float kMaxProbability = 1.f - kMinProbability;// 最大概率 0.9
constexpr float kMinCorrespondenceCost = 1.f - kMaxProbability;//最小通信成本
constexpr float kMaxCorrespondenceCost = 1.f - kMinProbability;//最大通信成本
...
/*
  将概率转换到[1,32767] 之间的数据
    hit 0.45 ==> (0.45 - 0.1)*32766/(0.9-0.1)+1
*/ 
