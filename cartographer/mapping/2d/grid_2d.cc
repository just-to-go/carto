...
void Grid2D::FinishUpdate() {
  while (!update_indices_.empty()) {
    DCHECK_GE(correspondence_cost_cells_[update_indices_.back()],
              kUpdateMarker);
    /*
      update_indices_ 存放更新概率的点.
        结束更新时候,将刚刚更新的地图概率,都减去 kUpdateMarker 值
    */ 
...
void Grid2D::ComputeCroppedLimits(Eigen::Array2i* const offset,
  ...
  *limits = CellLimits(known_cells_box_.sizes().x() + 1,
  ...
  //   <<"\n offset.x = "<<(*offset)(0)
  //   <<"\n offset.y = "<<(*offset)(1)
  //   <<"\n cell_limit.x = "<<known_cells_box_.sizes().x() + 1
  //   <<"\n cell_limit.y = "<<known_cells_box_.sizes().y() + 1;
}
...
/*
  扩展地图边界:
    point 边界点
    grids = {mutable_correspondence_cost_cells()} = {correspondence_cost_cells_}
    grids_unknown_cell_values = {0}
*/ 
void Grid2D::GrowLimits(const Eigen::Vector2f& point,
  ...
  while (!limits_.Contains(limits_.GetCellIndex(point))) {
    ...
    /*
      扩张地图:
        x,y方向分别扩大一倍.地图始终保持一个正方形,且正方形的中心就是地图的建立时候的原点
    */ 
    ...
    for (size_t grid_index = 0; grid_index < grids.size(); ++grid_index) {
      ...
      /*
        遍历旧地图尺寸,赋值从旧地图尺寸开始赋值,这样遍历完旧地图之后,刚好把原来地图的概率赋值完成,新扩张的部分,都初始化为0
      */ 
    ...
    if (!known_cells_box_.isEmpty()) {
      ...
      */ 
      ...
      //   <<"\n x_offset = "<<x_offset
      //   <<"\n y_offset = "<<y_offset
      //   <<"\n known_cells_box_.min.x = "<<known_cells_box_.min().array()[0]
      //   <<"\n known_cells_box_.min.y = "<<known_cells_box_.min().array()[1]
      //   <<"\n known_cells_box_.max.x = "<<known_cells_box_.max().array()[0]
      //   <<"\n known_cells_box_.max.y = "<<known_cells_box_.max().array()[1];
    }
  }
}
