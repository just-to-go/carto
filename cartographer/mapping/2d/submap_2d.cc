...
std::vector<std::shared_ptr<const Submap2D>> ActiveSubmaps2D::InsertRangeData(
  ...
  if (submaps_.empty() ||
    ...
    // 没有子地图　或者　后一个子地图插入雷达数据　等于　num_range_data(),需要重新创建一张地图
    /*
      重新创建一张子地图,地图原点,以该帧雷达数据的原点来创建.也即该帧雷达数据最后一个点对应时刻的Tmap_base(机器人在地图中的坐标)
    */ 
...
/*
  初始化 RangeDataInserterInterface 类:
    hit_table_ , miss_table_ 表初始化
*/ 
...
ActiveSubmaps2D::CreateRangeDataInserter() {
  ...
  if(flag == false)
  {
    ...
    LOG(WARNING)
      <<"\n ActiveSubmaps2D::CreateRangeDataInserter:"
      <<"\n options_.range_data_inserter_options().range_data_inserter_type() = "<<options_.range_data_inserter_options().range_data_inserter_type()
      <<"\n proto::RangeDataInserterOptions::PROBABILITY_GRID_INSERTER_2D = "<<proto::RangeDataInserterOptions::PROBABILITY_GRID_INSERTER_2D
      <<"\n proto::RangeDataInserterOptions::TSDF_INSERTER_2D = "<<proto::RangeDataInserterOptions::TSDF_INSERTER_2D;
  }
